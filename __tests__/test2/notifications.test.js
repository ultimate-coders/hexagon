...
describe('notifications test', ()=>{
  ...
  it('should create a notification', async ()=>{
    ...
    expect(res.body.message).toEqual('error: invalid input syntax for type uuid: \"id2\"');
  });
  
  it('should get all notifications', async ()=>{
    
   
    let res = await request.get('/api/v1/notifications').set('Authorization', `Bearer accessToken`);
  ...
  it('should update notification', async ()=>{
    ...
    expect(res.status).toEqual(500);
    expect(res.body.message).toEqual('error: invalid input syntax for type uuid: \"undefined\"');
  });
