...
describe('follow endpoint', ()=> {
  ...
  it('Will insert or update the follow data', async ()=> {
    ...
    let res = await request.post('/api/v1/follow').set('Authorization', `Bearer ${accessToken}`).send(test);

    expect(res.status).toEqual(200);
    // expect(res.body[0].follower).toEqual(1);
    // expect(res.body[0].following).toEqual(2);
